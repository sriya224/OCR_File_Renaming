...
def rename_mrxs_files(ocr_txt_path, mrxs_folder):
    ...
    rename_map = {}
    study_id_to_files = defaultdict(list)  # NEW: map study ID → list of original filenames

    for fname, lines in grouped.items():
        study_id, cleaned_name = extract_study_id_and_clean(lines)
        rename_map[fname] = cleaned_name
        if study_id:  # only include if we found a study ID
    ...
    # NEW: Identify outlier study IDs
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Rename .mrxs files using OCR results and flag outliers.")
