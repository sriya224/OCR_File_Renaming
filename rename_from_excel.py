...
def rename_from_excel(excel_path, mrxs_folder, log_path="rename_log_from_excel.txt"):
    df = pd.read_excel(excel_path)

    if "Original File Name" not in df.columns or "New File Name" not in df.columns:
        raise ValueError("Excel must contain 'Original File Name' and 'New File Name' columns.")
    ...
    with open(os.path.join(mrxs_folder, log_path), "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    rename_from_excel(args.excel, args.folder)
